    ...
    <script>
        async function runCheck() {
            ...
            try {
                ...
                html += row(!!data.tls.certificate, 'Server Certificate', data.tls.certificate || 'N/A');
                ...
                html += '</div>';
                html += '<div class="card"><h2>Database Connection (ODBC with TLS)</h2>';
                ...
                if (data.db.databases) html += rowInfo('Databases Found', data.db.databases.length + ' databases');
                if (data.db.error) html += row(false, 'Error', data.db.error);
                html += '</div></div>';
                const allGood = data.tls.connected && tlsOk && data.db.connected;
